...
存储协议是客户和存储miner之间的协议，用于在网络中存储一些数据。一旦交易开始，miner收到了要存储的数据，它将反复向供应链[证明](#证明)它一直按照协议存储数据，以便能够收取[奖励](../mine/mining-rewards.md)。如果不能做到，miner的 FIL 将被[削减](../mine/slashing.md)或失去。
